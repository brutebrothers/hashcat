...
#define CT_MAX_LEN_BASE64 (((3136 + 16) * 8) / 6) + 3
  ...
  token.len_min[1] = 44;
  token.len_max[1] = 44;
  token.attr[1] = TOKEN_ATTR_VERIFY_LENGTH | TOKEN_ATTR_VERIFY_BASE64A;
  ...
  token.attr[4] = TOKEN_ATTR_VERIFY_LENGTH | TOKEN_ATTR_VERIFY_BASE64A;
...
#define SALT_LEN_BASE64 ((32 * 8) / 6) + 3
  ...
  if (tmp_len < 12 || tmp_len > 16)
    return (PARSER_IV_LENGTH); //16
  ...
  if (tmp_len <= 16)
    return (PARSER_CT_LENGTH);

  tmp_len -= 16;

  if (tmp_len < 30 || tmp_len > 3136)
    return (PARSER_CT_LENGTH);
  ...
  for (u32 i = 0; i < j; i++)
    onepass->ct_buf[i] = byte_swap_32(onepass->ct_buf[i]);
...
#define CT_MAX_LEN_BASE64 (((3136 + 16) * 8) / 6) + 3
  ...
  for (u32 i = 0; i < j; i++)
    tmp_buf[i] = byte_swap_32(onepass->ct_buf[i]);
