...
u32 u8_to_u32(const u8* bytes) {
  u32 result = (bytes[0] << 24) + (bytes[1] << 16) + (bytes[2] << 8) + bytes[3];
  return result;
}

void u32_to_u8(const u32 u32, u8* u8) {
  u8[0] = (u32 & 0xff000000) >> 24;
  u8[1] = (u32 & 0x00ff0000) >> 16;
  u8[2] = (u32 & 0x0000ff00) >> 8;
  u8[3] = u32 & 0x000000ff;

  //printf("extracted %d=>%d,%d,%d,%d\n", u32, u8[0], u8[1], u8[2], u8[3]);
}

void u32_to_u8_rev(const u32 u32, u8* u8) {
  u8[3] = (u32 & 0xff000000) >> 24;
  u8[2] = (u32 & 0x00ff0000) >> 16;
  u8[1] = (u32 & 0x0000ff00) >> 8;
  u8[0] = u32 & 0x000000ff;
...
KERNEL_FQ void m90200_comp (KERN_ATTR_TMPS_ESALT (pbkdf2_sha256_tmp_t, pbkdf2_sha256_aes_gcm_t))
{
  ...
  u32 keybuf[8] = {
    salt_bufs[DIGESTS_OFFSET].salt_buf_pc[0],
    salt_bufs[DIGESTS_OFFSET].salt_buf_pc[1],
    salt_bufs[DIGESTS_OFFSET].salt_buf_pc[2],
    salt_bufs[DIGESTS_OFFSET].salt_buf_pc[3],
    salt_bufs[DIGESTS_OFFSET].salt_buf_pc[4],
    salt_bufs[DIGESTS_OFFSET].salt_buf_pc[5],
    salt_bufs[DIGESTS_OFFSET].salt_buf_pc[6],
  ...
  AES_GCM_Init (ukey_final, key_len, key, subKey, s_te0, s_te1, s_te2, s_te3, s_te4);
